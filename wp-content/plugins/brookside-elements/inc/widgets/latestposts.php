<?php
add_action('widgets_init','register_brookside_grid_post_posts_widget');
function register_brookside_grid_post_posts_widget(){
    register_widget('brookside_grid_posts_widget');
}

class brookside_grid_posts_widget extends WP_Widget{

    function __construct(){
        parent::__construct( 'brookside_grid_posts_widget','brookside-Mega Menu Posts',array('description' => 'Grid Posts widget to display in WP Mega Menu. Do not use it in your sidebar.'));
    }
        ...
        if( ! empty($instance['show_cat']) && $instance['show_cat'] == 'on' ){
            ...
            if( !empty($instance['category']) ){
                ...
                foreach ( $instance['category'] as $value ) {
                    $catName = __('All','brookside-elements');
                    if( $value != 'allpost' ){
                ...
                foreach ( $instance['category'] as $value ) {
                    ...
                    if( $data->have_posts()){
                        ...
                        if( $instance['show_nav'] == 'on' ){
                            $output .= '<span data-showcat="'.$instance['show_date'].'" data-type="post" data-category="'.$value.'" data-current="1" data-oderby="'.$instance["order_by"].'" data-oderby="'.$instance["order_by"].'" data-column="'.$no_column.'"  data-total="'.$data->max_num_pages.'" class="dashicons dashicons-arrow-left-alt2 wpmm-left wpmm-gridcontrol-left disablebtn"></span>';
            ...
            if( $data->have_posts()){
                ...
                if( ! empty($instance['show_nav']) && $instance['show_nav'] == 'on' ){
                    ...
                    $output .= '<span data-showcat="'.$instance['show_date'].'" data-type="post" data-category="'.$data_category.'" data-current="1" data-oderby="'.$instance["order_by"].'" data-oderby="'.$instance["order_by"].'" data-column="'.$no_column.'"  data-total="'.$data->max_num_pages.'" class="dashicons dashicons-arrow-left-alt2 wpmm-left wpmm-gridcontrol-left disablebtn"></span>';
                    $var = ($data->max_num_pages == 1)? 'disablebtn' : '';
    ...
    function form($instance)
    {
        ...
        ?>
        <p>
            <label for="<?php echo $this->get_field_id( 'order_by' ); ?>"><?php esc_html_e('Ordered By', 'brookside-elements'); ?></label>
            <?php
            $options = array(
                'popular' 	=> 'Popular',
                'latest' 	=> 'Latest',
        ...
        <p>
            <label for="<?php echo $this->get_field_id( 'category' ); ?>"><?php esc_html_e('Select Category', 'brookside-elements'); ?></label>
            <?php
            $options = array();
            $options['allpost'] = 'All';
        ...
        <p>
            <label for="<?php echo $this->get_field_id( 'no_column' ); ?>"><?php esc_html_e('Number of Column', 'brookside-elements'); ?></label>
            <?php
            $options = array(
                'col1' 	=> 'Column 1',
                'col2' 	=> 'Column 2',
        ...
        <p>
            <label for="<?php echo $this->get_field_id( 'count' ); ?>"><?php esc_html_e('Post Count', 'brookside-elements'); ?></label>
