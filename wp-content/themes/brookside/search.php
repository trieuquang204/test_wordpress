...
$sidebar = get_theme_mod('brookside_sidebar_pos_search', 'sidebar-right');
if($sidebar != 'none'){
...
<div id="page-wrap-blog" class="container">
	<div id="content" class="<?php echo esc_attr($sidebar_pos); ?>">
		<div class="row">
			...
			$columns = get_theme_mod( 'brookside_search_columns', 'span6');
	      	$display_categories = true;
	      	$display_date = get_theme_mod( 'brookside_display_post_date_search', true);
	      	$display_comments = get_theme_mod( 'brookside_display_post_comments_search', false);
	      	$excerpt_count = get_theme_mod( 'brookside_search_excerpt_count', '15' );
	      	if( function_exists('BrooksideGetPostViews')){
	      		$display_views = get_theme_mod( 'brookside_display_post_views_search', false);
	      	} else {
	      		$display_views = false;
	      	}
	      	if( function_exists('getPostLikeLink') ){
	      		$display_likes = get_theme_mod( 'brookside_display_post_likes_search', false);
	      	} else {
	      		$display_likes = false;
	      	}
	      	if( function_exists('brookside_calculate_reading_time') ){
	      		$display_read_time = get_theme_mod( 'brookside_display_post_read_time_search', false);
	      	} else {
	      		$display_read_time = false;
	      	}
	      	$pagination = get_theme_mod( 'brookside_display_post_pagination_search', 'standard');
	      	$display_readmore = 'true';
	      	$ignore_featured = $ignore_sticky_posts = true;
	      	$text_align = 'textleft';
			$out = '';
			$post_style = get_theme_mod('brookside_search_style', 'style_4');
		...
		$out .= '</div>';
			echo ''.$out;
				...
				<div class="span12">
					...
					<div class="text mb30">
						<?php esc_html_e('Sorry, but nothing matched your search terms. Please try again with some different keywords.', 'brookside'); ?>
					</div>
				</div>
