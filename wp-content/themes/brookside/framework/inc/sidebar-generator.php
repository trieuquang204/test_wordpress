...
class sidebar_generator {
			...
			<script>
				function add_sidebar( sidebar_name )
				{
					...
					var mysack = new sack("<?php echo site_url(); ?>/wp-admin/admin-ajax.php" );    
				  	...
				  	mysack.onError = function() { alert("<?php esc_html_e('Ajax error. Cannot add sidebar','brookside'); ?> ")};
				  	mysack.runAJAX();
					return true;
				}
	...
	public static function add_sidebar(){
		...
		if(isset($sidebars[$id])){
			die("alert('". esc_html__('Sidebar already exists, please use a different name.','brookside')."')");
		}
		...
		$js = "
			...
			//var textNode = document.createTextNode('[<a href=\'javascript:void(0);\' onclick=\'return remove_sidebar_link($name);\'>". esc_html__('Remove','brookside')."</a>]');
			//cellLeft.appendChild(textNode)
	...
	public static function remove_sidebar(){
		...
		$id = sidebar_generator::name_to_class($name);
		if(!isset($sidebars[$id])){
			die("alert('". esc_html__('Sidebar does not exist.','brookside')."')");
		}
	...
	public static function admin_page(){
		...
		<script>
			function remove_sidebar_link(name,num){
				answer = confirm("<?php esc_html_e('Are you sure you want to remove','brookside'); ?> " + name + "<?php esc_html_e('?\nThis will remove any widgets you have assigned to this sidebar.','brookside'); ?>");
				if(answer){
					//alert('AJAX REMOVE');
			...
			function add_sidebar_link(){
				var sidebar_name = prompt("<?php esc_html_e('Sidebar Name:','brookside'); ?>","");
				//alert(sidebar_name);
				add_sidebar(sidebar_name);
			}
		</script>
		<div class="wrap">
			<h2><?php esc_html_e('Sidebar Generator','brookside'); ?></h2>
			
			<p><?php esc_html_e('You can generate as many sidebars as you need. Then you can place them on any page you wish.','brookside'); ?></p>
			<br />
			<table class="widefat page" id="sbg_table" style="width:600px;">
				<tr>
					<th><?php esc_html_e('Name','brookside'); ?></th>
					<th><?php esc_html_e('CSS class','brookside'); ?></th>
					<th><?php esc_html_e('Remove','brookside'); ?></th>
				</tr>
				<?php
				$sidebars = sidebar_generator::get_sidebars();
				...
				<tr class="<?php echo ''.$alt?>">
					...
					<td><a href="javascript:void(0);" onclick="return remove_sidebar_link('<?php echo ''.$sidebar; ?>',<?php echo ''.$cnt+1; ?>);" title="<?php esc_attr_e('Remove this sidebar','brookside'); ?>"><?php esc_html_e('remove','brookside'); ?></a></td>
				</tr>
				<?php
						$cnt++;
					}
					...
					?>
					<tr>
						<td colspan="3"><?php esc_html_e('No Sidebars defined','brookside'); ?></td>
					</tr>
					<?php
				}
				?>
			...
			<div class="add_sidebar">
				<a href="javascript:void(0);" class="button" onclick="return add_sidebar_link()" title="<?php esc_attr_e('Add a sidebar','brookside'); ?>"><?php esc_html_e('Create Sidebar','brookside'); ?></a>
			</div>
		</div>
		<?php
	}
	...
	*/
	public static function save_form($post_id){
		...
		if(!empty($is_saving)){
			$page_sidebar = rwmb_get_value('brookside_page_sidebar');
	...
	<div id='sbg-sortables' class='meta-box-sortables'>
		<div id="sbg_box" class="postbox " >
			<div class="handlediv" title="<?php esc_attr_e('Click to toggle','brookside'); ?>"><br /></div><h3 class='hndle'><span><?php esc_html_e('Sidebars','brookside'); ?></span></h3>
			<div class="inside">
				<div class="sbg_container">
					<input name="sbg_edit" type="hidden" value="sbg_edit" />
					
					<p>
						<?php esc_html_e("Select the sidebar you wish to display on this page. If you don't want to display a Sidebar, select the 'Full-Width' Template.", "brookside"); ?>
						<br />
						<strong><?php esc_html_e('Note', 'brookside'); ?>:</strong><?php esc_html_e("You must first create the sidebar under Appearance > Sidebars.",'brookside'); ?>
					</p>
					<ul>
					<?php 
					...
					//var_dump($wp_registered_sidebars);		
						for($i=0;$i<1;$i++){ ?>
							...
							<select name="sidebar_generator[<?php echo ''.$i?>]" style="display: none;">
								<option value="0"<?php if($selected_sidebar[$i] == ''){ echo " selected";} ?>><?php esc_html_e('WP Default Sidebar','brookside'); ?></option>
							<?php
